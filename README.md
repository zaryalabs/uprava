...
Текущий baseline репозитория — `0.2.20`. Он добавляет foundation для Managed
Agent Work Loop: protocol spike выбрал experimental Codex app-server v2 из
`codex-cli 0.144.1`, подтвердил approvals, typed user input, interrupt,
reconnect/resume, safe policy and MCP credential boundary и зафиксировал
process-per-attempt topology. Managed mode ещё не реализован и не является
default. Baseline наследует `0.2.19` с durable Task Runs, isolated Git
...
реализованы двадцать implementation slices, workspace-centered UI follow-up, единый
