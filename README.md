...
- **Project Workspace Inspector** — рабочая поверхность вне чата: дерево и
  просмотр файлов, лёгкое редактирование текста, workspace-терминалы, diff,
  проверки и привязка evidence к trace.
...
Текущий baseline репозитория — `0.2.5` с Background Jobs поверх protocol v2,
продуктовой и архитектурной документацией. `V01` обозначает первый продуктовый
срез, выпущенный как `0.1.0`. После него реализованы восемь срезов очереди,
единый hardening-аудит, renderer/PTY-срез workspace и первый baseline
self-hosted CI/CD deployment.
...
- Project Workspace Inspector с безопасным чтением и сохранением текста,
  ограниченным выполнением команд, историей команд, diff и checks;
