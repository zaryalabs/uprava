...
Текущий baseline репозитория — `0.2.21`. Он добавляет shared contracts,
SQLite persistence и execution-policy foundation для Managed Agent Work Loop:
typed execution profiles, runtime attempts, provider interactions, immutable
effective policy с детерминированным hash и раздельный capability admission без
silent fallback. Node пока объявляет managed capabilities unavailable, поэтому
Managed mode ещё не реализован и не является default. Baseline наследует
`0.2.20` с Codex app-server v2 protocol gate и `0.2.19` с durable Task Runs, isolated Git
...
реализован двадцать один implementation slice, workspace-centered UI follow-up, единый
