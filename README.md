...
Текущий baseline репозитория — `0.2.17` с plugin-driven Visual Artifact System:
generic versioned artifact kernel, source-aware inline/block/artifact renderers,
bundled color, Mermaid/PlantUML, review и trace plugins поверх target-based
plugin contribution resolution и Markdown/Plain Text chain на Streamdown,
...
реализованы семнадцать implementation slices, workspace-centered UI follow-up, единый
...
путём Core/Web/ToolHive, предсказуемыми портами, volumes состояния, healthcheck
...
- Docker Compose dev profile для Core/Web/ToolHive и host Node Daemon/Codex.
...
Core/Web/ToolHive можно запустить через Docker Compose:
