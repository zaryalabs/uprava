...
Текущий baseline репозитория — `0.2.6` с workspace-centered Web UI и Background
Jobs поверх protocol v2. `V01` обозначает первый продуктовый срез, выпущенный
как `0.1.0`. После него реализованы восемь срезов очереди, workspace-centered UI
follow-up, единый hardening-аудит, renderer/PTY-срез workspace и первый baseline
