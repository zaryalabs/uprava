...
Текущий baseline репозитория — `0.2.10` с Git-aware Review surface,
прозрачным live agent timeline и модульными Core/Node runtime boundaries поверх
Causality/Trace UX, запрашиваемого Deduction, workspace-centered Web UI,
Background Jobs и protocol v2.
`V01` обозначает первый продуктовый срез, выпущенный как `0.1.0`. После него
реализованы десять срезов очереди, workspace-centered UI follow-up, единый
