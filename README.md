...
Текущий baseline репозитория — `0.2.23`. Он добавляет Core orchestration для
Managed Agent Work Loop: policy preview и immutable admission, атомарный
interaction lifecycle, ordered turn/runtime projection, actual-state reconnect,
recovery audit and bounded metrics. Baseline наследует Node-managed Codex
...
Managed mode остаётся opt-in до Web vertical slice и real-provider release
gate. Baseline наследует `0.2.21` с shared contracts, persistence and policy
