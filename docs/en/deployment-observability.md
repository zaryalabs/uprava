...
This document records the working position for production-like deployment of
Uprava on the Zarya server: **Core stack in Docker, Node Daemon on the host
through systemd**. The goal is a convenient deployment model without losing
transparency: Docker services remain visible through normal container logs, and
the bare-metal daemon becomes visible through OpenTelemetry logs and metrics.
...
Production shape for `zarya-main`:
...
Uprava Core stack in Docker
  ...
  versioned core.sqlite - product-owned 0.1.8 or 0.2.0 state slot
...
- emit the Core metrics listed above;
