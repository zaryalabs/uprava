...
строится через отдельный ToolHive bridge в Compose; Node остаётся на host и
обращается к bridge по ограниченному loopback HTTP contract.
...
код не попадает. Для `0.2.3` exception перепроверен: он остаётся
