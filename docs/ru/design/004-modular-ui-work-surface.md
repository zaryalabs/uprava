...
Для Stage 1/2 лучше начать с **internal extension architecture**, даже если
...
Stage 1 должен заложить internal extension architecture без полноценного
...
Главная цель Stage 1: сам Cortex должен быть написан так, будто он уже
расширяемый, даже если пользовательские plugins еще не подключаются.

### Stage 2 baseline

Stage 2 может сделать modularity visible:
