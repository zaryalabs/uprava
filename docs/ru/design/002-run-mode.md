...
Зафиксированная позиция для Stage 1: Cortex начинает с рабочей сессии с живым
...
Stage 1 начинается с **Persistent Runtime**: Node Daemon запускает Codex
...
runtime должен иметь управляемый lifetime. Stage 1 baseline: если у runtime
...
остаются архитектурно возможными, но не являются Stage 1 реализацией.
...
Stage 1 проверяет тезис: **Persistent Runtime + Node Daemon + Core UI дают
...
Это default для Stage 1 и first Codex implementation.

#### Runtime lifetime policy

Для Stage 1 не нужен сложный scheduler с большим набором lease-политик.
...
future bounded tasks, но Stage 1 не должен начинаться с нее.
...
вернуть review-ready output обратно в общую рабочую поверхность. Это не Stage
1, но Run Mode должен не закрывать такой путь.
...
- provider-neutral perfection across every CLI agent.
...
### Stage 1 implementation: Process-backed Interactive Session
...
Stage 1 не реализует bounded tasks, workflow engine, sandbox orchestration,
multi-agent scheduling или generic provider abstraction. Но модель должна быть
достаточно общей, чтобы эти режимы позже добавились как новые runtime strategies
или work contracts, а не как отдельный продукт.
...
Provider Adapter изолирует конкретный способ общения с Codex runtime.
...
Stage 1 может иметь один production adapter для Codex. Provider-neutral API
...
Stage 1 должен различать долговечные и live объекты.

#### Project

Product-level контейнер. В Stage 1 это может быть тонкая запись: name, repo или
...
Runtime step kinds для Stage 1:
...
Stage 1 не требует идеальной trace semantics. Нужно достаточно events, чтобы
ответить на вопросы:
...
Это намеренно простой baseline для Stage 1. Later versions могут добавить
...
Stage 1 events должны быть append-only и ordered per runtime session.
...
В Stage 1 diff не требует full checkpoint/rollback.
...
Stage 1 permission model может быть простой, но explicit:
...
- Which exact Codex runtime protocol should Stage 1 adapter use first:
...
- Do we need per-turn diff attribution in Stage 1, or is session-level diff
