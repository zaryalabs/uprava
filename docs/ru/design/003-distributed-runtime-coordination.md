...
Stage 1 не должен превращать это в workflow engine, scheduler or lock manager.
...
Stage 1 coordination строится вокруг пяти идей.
...
3. **Command proxy, not workflow scheduler**. Core dispatch-ит команды через
   ...
   открыть его через heartbeat response. Stage 1 не делает полноценный
...
2. **Coordination unit is `RuntimeSession`**, linked to `SessionThread` and
   `WorkspaceBinding`.
3. **Stage 1 is command proxy plus persisted event log**, not a full
   desired-state reconciler.
...
| `same workspace active` | Same workspace already has active runtime | Warn and allow override in Stage 1. |
...
- Stage 1 does not need global event ordering across all nodes.
...
- Should Stage 1 discover workspaces automatically, or should users register
...
- Should same workspace active runtime be warning or hard block for the first
  prototype?
- How much event gap recovery do we need in Stage 1?
