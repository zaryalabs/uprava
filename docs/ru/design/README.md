...
docs/ru/design/003-modular-ui-work-surface.md
docs/ru/design/004-dynamic-ui-from-agents.md
```
...
| A-005 | Visualization and artifacts | Какие визуализации нужны Cortex: diff, terminal replay, causality map, test report, UML, charts, dashboards, dependency graphs, forms? Как они соотносятся с artifacts, blocks and plugins? | Продуктовая и техническая карта visual artifacts, включая first release vs later. |
...
- Developer workbench - главный Stage 1 сценарий для `A-002 Run Mode`, `A-003 Modular UI and work surface` and `A-007 Go to cause and causality UX`.
- Workflow and harness - сценарный срез для длинной работы, который проверяет `A-002 Run Mode`, но не заменяет его.
- Integration UX - частный случай модульности, plugins/tools and visual blocks; его нужно раскрывать внутри `A-003`, `A-004`, `A-005` and `A-006`.
...
- как устроены ключевые механики Cortex: distributed architecture, run mode, modular UI, plugins/tools, dynamic UI and visual artifacts;
