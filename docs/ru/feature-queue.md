...
| --- | --- | --- | --- | --- |
| 0 | V01 Developer Node Workbench | Первый пригодный продуктовый срез | Current design baseline | High |
...
подключить node, начать persistent Codex-backed session, посмотреть workspace
state, использовать terminal, отредактировать file and review trace/diff.

**First useful slice:** Описан в [v01.md](v01.md).

**Target direction:** Сохранить первый продукт маленьким, но не закрывать system
model для providers, tools, plugins, visual artifacts, task runs, mobile and
team/cloud modes.
...
**Value:** Позволяет chat, trace, artifacts, review and agents ссылаться на одну
и ту же workspace evidence.

**First useful slice:** Stable references for file, file range, edit, terminal
session, command, output range, diff hunk, check result, artifact, turn and trace
event.
...
- Какой queue item должен быть первым post-V01 product hardening slice?
...
- Насколько маленькой может быть первая visual artifact system, чтобы она все
  еще меняла review experience?
- Какой task-based runtime slice полезен до full durable workflow state?
