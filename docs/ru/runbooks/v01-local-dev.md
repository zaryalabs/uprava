...
Daemon, Vite Web Control Panel, SQLite persistence, fake-provider path и первый
минимальный Codex provider adapter.
...
you need evidence, then delete broken local state or use `make compose-reset`
...
Use `make compose-up` when you want services attached in the foreground for
interactive debugging. `make compose-smoke` starts or rebuilds the Compose
profile in detached mode before running checks. Set `SMOKE_SKIP_COMPOSE_UP=1`
to probe an already running non-default profile.
...
Compose profile starts Core, Web and a synthetic Node Daemon that sends
heartbeats to Core. Compose enables `CORTEX_AUTO_APPROVE_ENROLLMENTS=true` for
the synthetic Node only. Fake-provider path remains the deterministic default,
...
- Core inventory contains synthetic `Compose Node`;
- synthetic Node has a validated `/workspace` placement;
- Core can create a fake-provider session on that placement;
- Node receives start command, returns runtime to `ready`, processes a turn and
  persists a fake-provider assistant message;
- Core can restart and reload the same session history and ready runtime state
  from SQLite while Compose Node remains running;
- Compose Node can restart, reconnect, and process another turn for the same
  persisted session;
- fake provider can emit a deterministic runtime error, Core persists the
...
`SMOKE_PROVIDER_ERROR_CHECK`, чтобы запускать тот же smoke check against a
...
state file and then heartbeats with that credential. Heartbeats include bounded
...
rejects future heartbeats from that node.
...
Current fake-provider path executes through the Node-side Provider Adapter
...
browser checks against the Compose Web service. Default E2E run uses mocked
Core snapshots for deterministic UI warning/degraded-state assertions. To run
the real Core/Web/Node fake-provider browser path against the same profile,
first run `make compose-smoke`, then run:
...
`http://127.0.0.1:5173` with `playwright-cli`, verify trusted profile banner,
inventory tree, workspace/session flow, warning/degraded states and inspector
actions, then collect `make compose-logs` output if a defect needs debugging.
