...
- Core с проектами, нодами, агентами, agent sessions и agent runs;
- Node Daemon на ноде, который умеет запускать агента и отдавать состояние;
- привязка agent session/run к проекту, workspace и задаче;
- два базовых execution modes: persistent session и task-based run;
- чат как один из интерфейсов к session/run;
- файловый браузер проекта/workspace;
- терминал или terminal output для окружения агента;
- просмотр изменений и diff;
- базовый trace для agent session/run;
- простая модель статусов: draft / running / blocked / needs review / accepted / rejected / deferred;
- ручной review loop: что изменилось, что проверено, что рискованно, что делать дальше;
- минимальный plugin/block contract для визуальных артефактов.
...
Подробный инвентарь уже придуманных фич и направлений вынесен в [feature-inventory.md](feature-inventory.md).
Подробные стадии развития продукта вынесены в [product-stages.md](product-stages.md).

### Stage 1. Developer Node Workbench

Первый usable продукт для developer workflow:

- Core Backend and Web Control Panel;
- одна или несколько нод с Node Daemon;
- persistent Codex session на ноде;
...
- минимальная форма Tool Registry, Plugin Registry and visual block/artifact contract.

Task-based sandbox mode не входит в первую версию. Архитектура должна оставить для него место, но первый продуктовый фокус - persistent developer workbench.

### Stage 2. Modular Developer Workbench

Сделать модульность реальной продуктовой возможностью:

- Tool Registry v1;
- Plugin Registry v1;
- integration adapter model;
- MCP adapter как один из путей интеграции;
- native adapters where MCP is not enough;
- первые integrations: git provider and Linear;
- basic permissions and tool call trace;
- minimal visual plugin/block API.

### Stage 3. Visual Agent Work Surface

Сделать визуализации главным отличием продукта:
...
- dashboard artifacts;
- dynamic UI. Forms generated by agents/plugins;
- embedded external views.

### Stage 4. Task-Based Agent Runtime

Добавить второй execution mode:
...
Соединить persistent sessions and task-based runs:
...
Расширить продукт до distributed/team platform:
...
Перенести модель на другие типы работы:

- research workflows;
- аналитика и dashboards;
- документы и презентации;
- финансы и мониторинг;
- knowledge base workflows;
- личные и командные агентские процессы.
