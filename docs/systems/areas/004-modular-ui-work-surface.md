...
Baseline `0.2.17` монтирует эти contracts в реальных work surfaces: Markdown
передает точные message ranges color/diagram inline renderers, timeline
делегирует подходящие blocks через Extension Host, а Context Inspector открывает
generic artifact detail через plugin-provided viewer. Core shell не импортирует
конкретные artifact schemas и всегда сохраняет source/metadata fallback.
