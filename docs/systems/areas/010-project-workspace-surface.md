...
### Реализованная композиция Workbench 0.2.6

Текущая primary workspace composition — IDE-like, но не full IDE:

```text
File tree | Source / Diff
----------+--------------
Terminal tabs across the workspace width
```

File tree, Monaco editor/diff и xterm PTY видны как одна bounded рабочая
поверхность с внутренним overflow. Source и Diff являются modes одной editor
pane; тяжёлые renderer chunks загружаются только при открытии соответствующей
функции. Drafts сохраняются при переходах между Agent, Workbench и Jobs, а
conflict требует явного Reload, Discard или Save decision.

Постоянный `Command` block и видимая command history удалены из primary
Workbench: произвольная ручная команда выполняется в PTY terminal. Bounded
command API, protocol types и persistence остаются в Core/Node для будущих
traceable checks и автоматизированных actions; это backend capability, а не
дублирующий terminal UI.
...
- workspace shell и tabs `Agent / Workbench / Jobs`;
- file tree;
...
В `0.2.6` Workbench компонует file tree, одну Source/Diff pane и terminal во
внутренне ограниченный grid. Изменение ширины после sidebar/Inspector toggle
передаётся Monaco/xterm через container resize/fit path.
