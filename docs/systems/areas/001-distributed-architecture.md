...
Этот документ фиксирует рабочую позицию по распределенной механике Uprava:
...
Uprava должен управлять агентами и рабочими окружениями, которые физически
...
Daemon показывает код вида `UPRAVA-ABCD-1234`. Пользователь открывает Core
...
дефолте около 5 секунд это приемлемо для V01. Для активной session channel
...
Для V01 worker agent не должен думать о distributed topology. Его рабочая
...
channel или то, что эта machine является одной из нод Uprava.
...
- позже internal Uprava agent or orchestrator может видеть node availability
...
- один Core URL;
- Docker Compose local profile for reproducible hardened Core/Web/ToolHive/Generated UI Builder startup,
  with explicit host Node/Codex paths for real local workspace access and
...
Долгоживущая, но отзывная идентичность daemon. Для V01 это random
...
Двунаправленное соединение, которое daemon открывает outbound в Core. На V01
...
Uprava не должен ограничиваться одним интерфейсом для всех типов связи.
...
WebSocket is the V01 default because it is full-duplex, browser-compatible
...
transport for Core <-> Node, but it should not be required for V01.
...
window. Для V01 важнее latest status and meaningful transitions.
...
Node token должен храниться в protected local config for V01. Later можно
...
- protected local config for V01;
...
V01 UI should include a Nodes surface:
...
- How transport-neutral should the V01 stream envelope be while WebSocket is
