...
- persistent Codex credential profiles с однократной авторизацией.
...
## Spike перед implementation slice

OpenSandbox является выбранным первым candidate, но его server API ещё должен
быть закреплён конкретной версией после короткого integration spike. Spike
считается успешным, если доказано следующее:
...
- Rust adapter может работать по OpenAPI/HTTP без upstream SDK;
- измерены idle service, idle sandbox and active Codex run overhead.

Если эти условия не выполняются, решение возвращается к сравнению внешних
