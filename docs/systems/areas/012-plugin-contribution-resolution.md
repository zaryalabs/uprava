...
Visual Artifact System `0.2.17` переиспользует тот же resolver для
selector-aware inline/block/artifact-viewer targets и для `artifact.type`.
Поэтому новый artifact family не получает скрытого privileged winner:
конфликтующие declarations видимы и управляются теми же revisioned
order/disable preferences, а отсутствие active winner запрещает новые
artifact mutations, не удаляя уже сохраненные versions and fallbacks.
