...
Implementation baseline `0.2.22` реализует Node-owned managed runtime, но ещё
не замыкает целевую Core/Web форму: Codex app-server остаётся живым между
turns, а существующий adapter `codex exec/resume` сохранён отдельно. Пункт очереди
...
Это ещё не default-on gate: Core orchestration, Web interaction cards и реальная
acceptance/recovery matrix закрываются этапами 3–5.
