...
Implementation baseline `0.2.21` ещё не является этой целевой формой:
Codex adapter запускает `codex exec` или `codex exec resume` на каждый turn и
восстанавливает continuity через provider resume reference. Пункт очереди
...
Эта foundation не меняет runtime default и не является opt-in managed vertical
slice: следующий gate — Node-managed Codex runtime.
