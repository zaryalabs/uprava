...
Implementation baseline `0.2.23` реализует Node-owned managed runtime и
Core-owned orchestration, но ещё не замыкает целевую Web форму: Codex app-server
остаётся живым между turns, а существующий adapter `codex exec/resume` сохранён отдельно. Пункт очереди
...
Это закрывает этап 3, но не Web work surface, rollout и real-provider acceptance
этапов 4–5.
