...
repository или CI logs. `toolhive.env` содержит Docker group id и пароль
локального encrypted secrets store; deploy проверяет права `0600`, соответствие
group id Docker socket и отсутствие example password, не выводя значения.

OpenSandbox получает `/srv/uprava-workspaces` как bind mount с тем же
абсолютным target; иначе server-side path validation и Docker daemon будут
видеть разные пути. Его SQLite живёт в
`/opt/apps/uprava/state/opensandbox`, а API доступен host Node только через
`127.0.0.1:18083`.
...
/opt/apps/uprava/
  ...
  state/toolhive/
  state/opensandbox/opensandbox.db
  backups/pre-deploy/<candidate-release-id>/core.sqlite
...
Core/Web/ToolHive/Generated UI Builder/Node/Codex task image refs и checksum извлечённого Node
binary. Manifest не
...
- `build` использует только host Docker engine для build, startup check и
  publication immutable artifacts, включая Codex task image, затем создаёт
  release manifest.
- `deploy` создаёт directories и ops files, устанавливает systemd unit и
  manifest, создаёт один проверенный online backup Core SQLite для immutable
  candidate id, сохраняет согласованные manifest/binary links активного release
  как rollback target, pull-ит artifacts, проверяет Node checksum, запускает
  Core/Web/ToolHive/Generated UI Builder/OpenSandbox и перезапускает Node. Он не проверяет health,
  не чистит artifacts и
  не сбрасывает state.
- `finalize` ждёт local и public health Core/Web, проверяет public Git SHA,
  ...
  readiness gate сначала останавливает candidate Core, проверяет checksum и
  integrity pre-deploy backup, атомарно восстанавливает Core SQLite, затем
  возвращает сохранённые links и перезапускает предыдущие Core/Web/Node.
  Rollback разрешён только для той же release family и тех же state slots. Если
  backup отсутствует, повреждён, это первая установка или target небезопасен,
...
ToolHive запускается отдельным profile, использует digest-pinned image,
loopback ports, private network, persistent state и Docker socket с проверенным
supplementary group id. Старый manifest без profile остаётся parseable для
rollback.

OpenSandbox является исключением: для Docker lifecycle ему нужен host Docker
socket. Он публикуется только на loopback, ограничивает host mounts
`/srv/uprava-workspaces` и удаляет sandbox по explicit cleanup/TTL. До внедрения
API key и persistent Codex credentials этот профиль не считается production
security boundary.
...
Retention удаляет только старые Uprava release manifests/directories, связанные
с удаляемыми candidate pre-deploy backups и images
из Uprava repositories. Global Docker prune не выполняется. Phase
