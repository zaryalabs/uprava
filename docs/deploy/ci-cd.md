...
1. собирает production images Core, Web, Node и ToolHive;
...
5. применяет Core/Web/ToolHive через Compose;
...
1. ожидает health Core, Web и ToolHive и сверяет ToolHive `0.40.0`;
...
а Core/Web/Node/ToolHive перезапускаются. Первый переход совместим с legacy
manifest без ToolHive profile; после перехода rollback требует тот же ToolHive
state slot. При отсутствии безопасного target failed first candidate
...
Обычный delivery никогда не удаляет Core, Node или ToolHive state. Release manifest не
