...
3. создаёт и проверяет immutable pre-deploy online backup Core SQLite;
4. сохраняет активный release как проверяемый rollback target;
5. активирует candidate и скачивает digest-pinned artifacts;
6. применяет Core/Web/ToolHive/Generated UI Builder через Compose;
7. устанавливает и перезапускает Node binary через разрешённый systemd unit.
...
5. при ошибке readiness проверяет и восстанавливает candidate-specific
   pre-deploy Core backup, затем возвращает совместимый предыдущий release либо
...
только после восстановления pre-deploy Core state, а Core/Web/Node
перезапускаются. При отсутствии backup или безопасного target failed candidate
останавливается и active links удаляются. Ошибка retention после
