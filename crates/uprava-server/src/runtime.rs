...
const PUBLIC_RATE_WINDOW_SECONDS: i64 = 60;
const PUBLIC_GLOBAL_RATE_LIMIT: usize = 1_000;
const PUBLIC_PEER_RATE_LIMIT: usize = 120;
...
async fn enforce_public_rate(state: &AppState, key: &str, limit: usize) -> Result<(), AppError> {
    let now = Utc::now();
...
async fn correlation_response_header(mut request: Request, next: Next) -> Response<Body> {
...
async fn workspace_file_write_route(
    State(state): State<Arc<AppState>>,
    headers: HeaderMap,
    Path(placement_id): Path<String>,
...
async fn workspace_command_run_route(
    State(state): State<Arc<AppState>>,
    headers: HeaderMap,
    Path(placement_id): Path<String>,
...
async fn workspace_command_history_route(
    State(state): State<Arc<AppState>>,
    Path(placement_id): Path<String>,
...
async fn workspace_file_write_with_correlation(
    state: &AppState,
    placement_id: ProjectPlacementId,
...
async fn workspace_command_run_with_correlation(
    state: &AppState,
    placement_id: ProjectPlacementId,
...
async fn workspace_diff_with_correlation(
    state: &AppState,
    placement_id: ProjectPlacementId,
    correlation_id: CorrelationId,
...
async fn dispatch_workspace_command<T>(
    state: &AppState,
    placement: &ProjectPlacementSummary,
    kind: CommandKind,
...
fn typed_workspace_command_payload(
    kind: CommandKind,
...
fn ensure_placement_intervention_allowed(
    placement: &ProjectPlacementSummary,
) -> Result<(), AppError> {
...
async fn submit_turn_for_actor(
    state: &AppState,
    session_id: SessionThreadId,
    content: String,
    correlation_id: CorrelationId,
...
async fn ensure_provider_quota_admission(
    state: &AppState,
    provider: &str,
    force: bool,
    operation: &str,
...
fn validate_job_timezone(timezone: &str) -> Result<(), AppError> {
    let timezone = timezone.trim();
...
struct JobRunConfigSnapshot {
    ...
    schedule: Option<JobSchedule>,
    timezone: String,
    overlap_policy: JobOverlapPolicy,
    continue_after_error: bool,
}

fn snapshot_for_job(job: &JobDetail) -> JobRunConfigSnapshot {
...
async fn load_job_detail(state: &AppState, job_id: &JobId) -> Result<JobDetail, AppError> {
    let row = sqlx::query(
        r#"
...
async fn load_job_runs(state: &AppState, job_id: &JobId) -> Result<Vec<JobRunSummary>, AppError> {
...
async fn load_job_run(state: &AppState, job_run_id: &JobRunId) -> Result<JobRunSummary, AppError> {
    let row = sqlx::query(
        r#"
...
fn parse_job_run_state(value: &str) -> JobRunState {
    match value {
...
fn spawn_job_scheduler(state: std::sync::Weak<AppState>) {
    tokio::spawn(async move {
        loop {
...
async fn recover_job_runs(state: &AppState) -> Result<(), AppError> {
    sqlx::query(
...
async fn dispatch_due_jobs(state: &AppState) -> Result<(), AppError> {
...
async fn claim_due_job(state: &AppState, job_id: &JobId) -> Result<(), AppError> {
    let job = load_job_detail(state, job_id).await?;
    if !job.job.enabled {
        return Ok(());
    }
...
async fn drive_job_runs(state: &AppState) -> Result<(), AppError> {
...
async fn start_job_run(state: &AppState, run: &JobRunSummary) -> Result<(), AppError> {
...
async fn drive_starting_job_run(state: &AppState, run: &JobRunSummary) -> Result<(), AppError> {
...
async fn drive_running_job_run(state: &AppState, run: &JobRunSummary) -> Result<(), AppError> {
    let Some(session_id) = run.session_thread_id.as_ref() else {
        return fail_job_run(
            state,
            run,
...
async fn update_command_result(
    state: &AppState,
    command_id: &CommandId,
    command_state: CommandState,
    result_payload: &JsonValue,
...
fn record_event_metrics(state: &AppState, event: &EventEnvelope) {
...
fn rotate_client_logs(path: &std::path::Path) -> std::io::Result<()> {
...
fn parse_command_state(value: &str) -> CommandState {
    match value {
